...
def retrieval():
    ...
    for i in range(len(w)):
        ...
        with open(file_n, 'a+', newline='') as csvFile:
            ...
            r1=['Acc. ID', 'Host', 'Country', 'Date of coll.', 'rejected a. acids', 'pr']
    ...
    for j1 in w:
        ...
        plt.plot(list(range(1, len(pv)+1)), list(pv))
        plt.xlabel('starting point of window')
        plt.ylabel('stretch variability')
        plt.xticks(np.arange(0, 676, 100))
        plt.yticks(np.arange(0, 6, 1))
        plt.savefig(f'{out_path}/{name}/variability_plot.png')
        plt.close()
    for j1 in w:
        name=str(j1)
        file_n=f'{out_path}/{name}/record.csv'
...
tk.Button(win, width=12, text='Quit', command=win.destroy, foreground='brown', relief='ridge', font=('Segoe Script', '14'), highlightthickness=4, highlightbackground='red', highlightcolor='white').place(x=38.75, y=615)
tk.Button(win, width=12, text='Submit', command=retrieval, foreground='dark green', relief='ridge', font=('Segoe Script', '14'), highlightthickness=4, highlightbackground='green', highlightcolor='white').place(x=300.25, y=615)
